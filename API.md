...
`GameClient.readRewards?(): Promise<FriendRewardsSnapshot>` is read-only and separate from `read()`'s simulated ledger. The host binds it to the selected verified Generations NFT. `GameHost` / `ConnectedGameHost` accept optional `linkedGenesisId: bigint`; CLI `game.json` accepts the positive decimal string equivalent. A linked Genesis is included only after verifying the same owner at the snapshot block. By default this does not replace Generations eligibility. The explicit `allowGenesisPreview: true` host/CLI option allows the configured Genesis to be selected for simulated companion play, verified with `readGenesisEligibility`. Live deployments always reject Genesis. `GameFriend`, `GameComponentProps`, and reward snapshots carry optional `collection: "genesis" | "generations"`; omitted means Generations. Picker identity and save namespaces distinguish collections even when token IDs match. Genesis reward reads have `collection: "genesis"` and no duplicate nested Genesis record.
