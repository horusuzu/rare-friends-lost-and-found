...
export function scoreIntent(result:ScoreShare,friendId:bigint,collection:string,gameName='Rare Invaders'){
 ...
 const text=game.title==='RARE DROP'
  ?(lang==='ja'?`RARE DROPで${pilot}と${score}点！ ${wave===game.maxWave?'最後の一粒で、Friendが生まれた！':`TIER ${wave}/${game.maxWave}`}\nあなたのFriendで挑戦してみて。`:`I scored ${score} with ${pilot} in RARE DROP! ${wave===game.maxWave?'Merged all the way to my Friend!':`Reached tier ${wave}/${game.maxWave}`}\nYour Friend. Your turn.`)
