...
function WalletViewport({ session, publicClient, ...props }: Omit<GameHostProps, "walletProvider" | "publicClient"> & {
  ...
  useEffect(() => {
    ...
    void Promise.allSettled([
      ...
      const errors = timedOut ? ["Friendの読み込みに時間がかかりすぎました。接続を確認して、もう一度お試しください。"] :
        [generations.status === "rejected" ? "Generationsを読み込めませんでした。再読み込みしてください。" : "", genesis.status === "rejected" ? "Genesisを確認できませんでした。再読み込みしてください。" : ""].filter(Boolean);
