...
function LifeGame({friendId,collection="generations",client,paused}:GameComponentProps){
 ...
 <LifeScene onBank={()=>setModal('rewards')} bankDisabled={disabled} life={life} sprites={sprites} reaction={paused?'':reaction} onPet={()=>{if(disabled)return;setBubble(greeting(life));setReaction(reaction?'':'happy');}}/>
 ...
 <div className="friend-caption">{collection==='genesis'?'Genesis':'Friend'} #{String(friendId)} <span>·</span> あなたと暮らす、世界にひとりのFriend</div>
 </section>
 ...
 <button type="button" className="home-piggy" aria-label="貯金箱を見る" disabled={disabled} onClick={()=>setModal('rewards')}><PiggyIcon/><span><b>この家の、貯金箱。</b><small>{realRewards ? `前回確認・未受取 ${tokenAmount((realRewards.genesis??realRewards).claimableRF)} RF · ${realRewards.genesis?`Genesis #${realRewards.genesis.tokenId}`:`${collection==='genesis'?'Genesis':'Friend'} #${friendId}`}` : 'NFTの本当のおこづかいを、のぞいてみよう。'}</small></span><i>↗</i></button><div className="daily-note"><span>✎ ふたりのメモ</span><p>{life.journal[0]?.text??'まだ名前しか知らないふたり。ご飯を食べたり、外へ出たり。一緒の時間から始めよう。'}</p></div>
 ...
 {modal&&<div className="life-overlay"><div className="life-dialog" ref={dialogRef} role="dialog" aria-modal="true" aria-label={modal==='rewards'?'NFTの貯金箱':modal==='card'?'思い出のポストカード':'暮らしの設定'} inert={paused||undefined}><button className="close-dialog" onClick={()=>setModal(null)}>閉じる</button>{modal==='rewards'?<RewardPanel client={client} friendId={friendId} previous={realRewards} onRead={setRealRewards} paused={paused}/>:modal==='card'?<><img className="memory-image" src={cardImage} alt={`${life.name}との思い出のポストカード`}/><p>画像を長押し・右クリックで保存できます。</p></>:<><span className="tiny">MAKE YOURSELF AT HOME</span><h2>きみとの暮らし。</h2><label>Friendの呼び名<input value={nickname} maxLength={20} onChange={e=>setNickname(e.target.value)}/></label><button className="main-action" disabled={disabled||!nickname.trim()} onClick={()=>{change(s=>rename(s,nickname));setModal(null);}}>この名前で呼ぶ</button><h3>スマートフォンで遊ぶ</h3><p>ブラウザーのメニューから「ホーム画面に追加」。育成データはこのブラウザーに保存されます。</p><p>起動時に対応ウォレットでの接続が必要です。ホーム画面版にウォレットがない場合は、いつものウォレット対応ブラウザーで開いてください。別ブラウザー・別端末には保存は引き継がれません。</p><h3>お部屋の模様替え</h3><button disabled={disabled||!economy||economy.consumables>0n||economy.rfBalance<2n*10n**18n} onClick={()=>void buyFrame()}>{economy&&economy.consumables>0n?'金色の額縁を使用中':'金色の額縁 · 2 demo RF'}</button><p className="hint">SDKの模擬購入です。お部屋の枠だけが変わり、この接続中のみ有効。育成と島づくりは無料です。</p><h3>暮らしのルール</h3><p>「おやすみ」で次の朝に進みます。閉じている間に弱ったり、いなくなったりはしません。データ削除で思い出も消えるため、このブラウザーを使い続けてください。</p><p className="hint">無料の育成プレビュー。材料・育成結果は模擬で、RFへの交換やNFTの発行はありません。</p></>}</div></div>}
 </section>;
}
