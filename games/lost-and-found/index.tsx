...
function LifeGame({friendId,collection="generations",client,paused}:GameComponentProps){
 ...
 const showCard=(card:Card)=>{try{setCardImage(memoryImage(card,life,sprites,language));setModal('card');}catch(e){setBubble(e instanceof Error?e.message:t("画像を作れませんでした。"));}};
 ...
 <LifeScene language={language} onBank={()=>setModal('rewards')} bankDisabled={disabled} life={life} sprites={sprites} reaction={paused?'':reaction} onPet={()=>{if(disabled)return;setBubble(greeting(life));setReaction(reaction?'':'happy');}}/>
 ...
 <div className="home-actions"><button aria-label={t("ごはん")} disabled={disabled} onClick={()=>setFood(!food)} aria-expanded={food}><span>🍞</span><b>{t("ごはん")}</b><small>{t("いっしょに、いただきます")}</small></button><button aria-label={t("おさんぽ")} disabled={disabled||life.energy<15} onClick={()=>change(s=>act(s,'walk'),'walk')}><span>🐾</span><b>{t("おさんぽ")}</b><small>{t("おうちのまわりを、てくてく")}</small></button><button aria-label={t("おやすみ")} disabled={disabled} onClick={()=>change(s=>act(s,'sleep'),'sleep')}><span>☾</span><b>{t("おやすみ")}</b><small>{t("次の朝へ。げんきを回復")}</small></button></div>
 ...
 <button type="button" className="home-piggy" aria-label={t("貯金箱を見る")} disabled={disabled} onClick={()=>setModal('rewards')}><PiggyIcon/><span><b>{t("この家の、貯金箱。")}</b><small>{realRewards ? t(`前回確認・未受取 ${tokenAmount((realRewards.genesis??realRewards).claimableRF)} RF · ${realRewards.genesis?`Genesis #${realRewards.genesis.tokenId}`:`${collection==='genesis'?'Genesis':'Friend'} #${friendId}`}`) : t("NFTの本当のおこづかいを、のぞいてみよう。")}</small></span><i>↗</i></button><div className="daily-note"><span>{t("✎ ふたりのメモ")}</span><p>{t(life.journal[0]?.text??("まだ名前しか知らないふたり。ご飯を食べたり、外へ出たり。一緒の時間から始めよう。"))}</p></div>
 ...
 <nav className="life-nav" aria-label={t("島ぐらしメニュー")}>{[['home','⌂',t("おうち")],['outings','☀',t("おでかけ")],['island','♧',t("島づくり")],['memories','✉',t("思い出")]].map(([id,icon,label])=><button key={id} aria-label={t(String(label))} aria-current={tab===id?'page':undefined} disabled={!atHome||saving} onClick={()=>{setTab(id);setFood(false);}}><span>{icon}</span>{label}</button>)}</nav>
