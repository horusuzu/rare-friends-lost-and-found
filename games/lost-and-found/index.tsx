...
function LifeGame({friendId,client,paused}:GameComponentProps){
 ...
 <button type="button" className="home-piggy" aria-label="貯金箱を見る" disabled={disabled} onClick={()=>setModal('rewards')}><PiggyIcon/><span><b>この家の、貯金箱。</b><small>{realRewards ? `前回確認・未受取 ${tokenAmount((realRewards.genesis??realRewards).claimableRF)} RF · ${realRewards.genesis?`Genesis #${realRewards.genesis.tokenId}`:`Friend #${friendId}`}` : 'NFTの本当のおこづかいを、のぞいてみよう。'}</small></span><i>↗</i></button><div className="daily-note"><span>✎ ふたりのメモ</span><p>{life.journal[0]?.text??'まだ名前しか知らないふたり。ご飯を食べたり、外へ出たり。一緒の時間から始めよう。'}</p></div>
