/**
 * A tiny WebAudio cue player: every sound is synthesised (oscillators and filtered noise), no audio files.
 * The AudioContext is created only by unlock(), which the game calls from a user gesture. Voices are
 * capped, every tone has a soft attack and release, and the player falls silent while muted, paused
 * or hidden. A browser without WebAudio simply plays nothing.
 */
