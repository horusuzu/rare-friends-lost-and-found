...
for(const [width,height] of sizes)console.log(await testGame('./games/rare-stickers',{width,height,screenshot:`./artifacts/stickers-${width}.png`,check:async({page,game})=>{
 ...
 await game.getByRole('button',{name:'シール帳',exact:true}).click();const placed=game.locator('.placed');assert.equal(await placed.count(),3);
 ...
 await game.getByRole('button',{name:'English',exact:true}).click();await game.getByRole('button',{name:/^Book/}).click();assert.equal(await game.locator('.placed').count(),3);await game.getByRole('button',{name:/^Packs 0/}).waitFor();
