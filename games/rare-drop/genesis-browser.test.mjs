...
for(const width of [390,1100]){
 const dir=await mkdtemp(join(tmpdir(),'genesis-drop-'));let server,browser;
 try{
 const build=await buildGame('./games/rare-drop',{outdir:join(dir,'dist')});server=createGameServer(build.outdir);await new Promise(r=>server.listen(0,'127.0.0.1',r));const origin=`http://127.0.0.1:${server.address().port}`;
 browser=await chromium.launch({headless:true,...(process.env.DROP_CHROMIUM?{executablePath:process.env.DROP_CHROMIUM}:{})});
 ...
 await game.getByRole('button',{name:'はじめる',exact:true}).click();assert.ok(fixture.genesisReads>=2);
 ...
 await page.getByRole('button',{name:'Choose Friend',exact:true}).click();await page.getByRole('button',{name:/^Friend #7730/}).click();await game.getByRole('button',{name:'はじめる',exact:true}).waitFor();
