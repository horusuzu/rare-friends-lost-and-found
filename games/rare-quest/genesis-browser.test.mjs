...
const widths=process.env.QUEST_SIZE?JSON.parse(process.env.QUEST_SIZE):[390,1100];for(const width of widths){
 const dir=await mkdtemp(join(tmpdir(),'genesis-quest-'));let server,browser;
 try{
 const build=await buildGame('./games/rare-quest',{outdir:join(dir,'dist')});server=createGameServer(build.outdir);await new Promise(r=>server.listen(0,'127.0.0.1',r));const origin=`http://127.0.0.1:${server.address().port}`;
 browser=await chromium.launch({headless:true,...(process.env.QUEST_CHROMIUM?{executablePath:process.env.QUEST_CHROMIUM}:{})});
 ...
 await page.getByRole('button',{name:'Choose Friend',exact:true}).click();await page.getByRole('button',{name:/^Friend #7730/}).click();await game.getByRole('button',{name:'はじめから',exact:true}).waitFor();
 ...
 assert.deepEqual(errors,[]);assert.deepEqual(fixture.errors,[]);const requests=await page.evaluate(()=>window.__friendWalletTest.state.requests);assert.ok(requests.every(m=>['eth_accounts','eth_requestAccounts','eth_chainId'].includes(m)));console.log(`Genesis selection, new game and transferred-owner rejection PASS ${width}`);
