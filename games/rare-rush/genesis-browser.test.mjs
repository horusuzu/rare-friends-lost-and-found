...
for(const width of [390,1100]){
 const dir=await mkdtemp(join(tmpdir(),'genesis-rush-'));let server,browser;
 try{
 const build=await buildGame('./games/rare-rush',{outdir:join(dir,'dist')});server=createGameServer(build.outdir);await new Promise(r=>server.listen(0,'127.0.0.1',r));const origin=`http://127.0.0.1:${server.address().port}`;
 browser=await chromium.launch({headless:true,...(process.env.RUSH_CHROMIUM?{executablePath:process.env.RUSH_CHROMIUM}:{})});
 ...
 await game.getByRole('button',{name:'乗車する',exact:true}).click();assert.ok(fixture.genesisReads>=2);
 ...
 await page.getByRole('button',{name:'Choose Friend',exact:true}).click();await page.getByRole('button',{name:/^Friend #7730/}).click();await game.getByRole('button',{name:'乗車する',exact:true}).waitFor();
