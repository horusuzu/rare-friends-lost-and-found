...
export default function Invaders(props:GameComponentProps){return <Arcade key={String(props.friendId)} {...props}/>;}
function Arcade({friendId,client,paused}:GameComponentProps){
 ...
 useEffect(()=>{alive.current=true;setReady(false);setError('');let current=true;void(async()=>{
  await client.read();const [sprite,raw]=await Promise.all([createFriendReader().read(friendId),client.loadLocal?.().catch(()=>{if(current)setSaveError(true);return null;})]);
  ...
  setReady(true);
 })().catch(()=>{if(current)setError('Could not load your Friend. Please retry.');});return()=>{current=false;alive.current=false;};},[client,friendId,attempt]);
